...
function Login() {
  ...
  const handleResgister = async (e) => {
    ...
    if (!captureInputs.email || !captureInputs.password) {
      ...
      const res = await login(captureInputs.email, captureInputs.password);
      setCaptureInputs({
        email: "",
        password: "",
      });
    ...
    <div className="login">
      ...
      <div className="login__content">
        <h1 className="login__title">
          {validarEstado ? "Registrate" : "Inicia sesion"}
        </h1>
        <form className="login__form">
          ...
          <div className="login__alert">{viewAlert && viewAlert}</div>
          <button
            className="login__btn login__btn--register"
            onClick={handleResgister}
          >
            {validarEstado ? "Registrate" : "Iniciar sesion"}
          </button>
          <p className="login__cambio" onClick={handleEstado}>
            {validarEstado
